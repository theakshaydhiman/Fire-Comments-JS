/*
* Fire Comments JS v0.1.0 (https://github.com/theakshaydhiman/Fire-Comments-JS)
...
let slugify = text => text.toString().toLowerCase().trim().replace(/&/g, '-and-').replace(/[\s\W-]+/g, '-').replace(/[^a-zA-Z0-9-_]+/g,'');
...
const commentsReplyRef = firebase.database().ref('replies').child(slugify(window.location.pathname));
...
commentsRef.on('child_added', snap => {
  ...
  html += "<div class='left'><img class='author-grav' src='https://www.gravatar.com/avatar/" + c.md5Email + "?s=80&d=retro'/></div>";
  html += "<h3>" + xssFilters.inHTMLData(c.name) + "</h3>";
  html += "<small>" + timeago().format(c.postedAt) + "</small>";
  html += "<p>" + xssFilters.inHTMLComment(c.message) + "</p>";
  html += "<button id='creply' class='comment-reply btn'><span id='ckey' style='display:none;'>" + key + "</span>Reply</button>";
  html += "</div>";
  ...
  thisReplyRef.on('child_added', snap => {
    ...
    html += "<div class='left'><img class='author-grav' src='https://www.gravatar.com/avatar/" + r.md5Email + "?s=80&d=retro'/></div>";
    html += "<h3>" + xssFilters.inHTMLData(r.name) + "</h3>";
    html += "<small>" + timeago().format(r.postedAt) + "</small>";
    html += "<p>" + xssFilters.inHTMLComment(r.message) + "</p>";
    html += "<button id='creply' class='comment-reply btn'><span id='ckey' style='display:none;'>" + key + "</span>Reply</button>";
    html += "</div>";
